...
const TAI_TO_TAIWAN_REPLACEMENTS = [
  ['台北', '臺北'],
  ['台中', '臺中'],
  ['台南', '臺南'],
  ['台東', '臺東'],
...
export function normalizeTaiwanPlaceName(name = '') {
  ...
  for (const [source, target] of TAI_TO_TAIWAN_REPLACEMENTS) {
...
export function normalizeTaiwanPlaceList(values = []) {
  return values.map((value) => normalizeTaiwanPlaceName(value)).filter(Boolean);
}
